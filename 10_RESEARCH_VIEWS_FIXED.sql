-- ============================================================================
-- 10 INTUITIVE RESEARCH VIEWS FOR INVESTOR DATABASE - FIXED VERSION
-- This version handles any existing table conflicts
-- ============================================================================

-- First, clean up any conflicting objects
DROP TABLE IF EXISTS network_connections CASCADE;
DROP VIEW IF EXISTS network_connections CASCADE;
...
-- 9. 🔗 INVESTOR NETWORK VIEW
-- Explore relationships between investors (renamed to avoid conflicts)
CREATE OR REPLACE VIEW investor_network AS
...
-- ============================================================================
SELECT 'View created: ' || table_name as status
FROM information_schema.views 
WHERE table_schema = 'public' 
AND table_name IN (
    'investor_profiles', 'firm_analysis', 'investment_activity', 
    'geographic_investment_map', 'sector_focus_analysis', 'stage_preferences_analysis',
    'top_performers', 'diversity_metrics', 'investor_network', 'comprehensive_search'
)
ORDER BY table_name;